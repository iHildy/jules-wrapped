...
/**
 * Postinstall script for jules-wrapped
 *
...
function findBinary(packageName) {
  const binaryName = os.platform() === "win32" ? "jules-wrapped.exe" : "jules-wrapped";
...
async function main() {
  try {
    ...
    if (!packageName) {
      console.error(`jules-wrapped: Unsupported platform: ${os.platform()}-${os.arch()}`);
      console.error("Please download the binary manually from:");
      console.error("https://github.com/iHildy/jules-wrapped/releases");
      process.exit(0); // Exit gracefully
    }

    console.log(`jules-wrapped: Detected platform package: ${packageName}`);
    ...
    if (!result) {
      ...
      if (basePackage !== packageName) {
        console.log(`jules-wrapped: Trying fallback package: ${basePackage}`);
      ...
      console.error(`jules-wrapped: Could not find binary for ${packageName}`);
    ...
    console.error("jules-wrapped: Postinstall error:", error.message);
